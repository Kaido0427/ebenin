@php
    ...
    $activeFilters = collect([
        'blog'      => $organizationId ?? null,
        'rubrique'  => $rubriqueId ?? null,
...
@section('meta_description', 'Recherchez des articles sur tous les blogs du réseau E-Benin')
...
<div style="background:var(--dark);padding:36px 0 0;">
    <div class="container">
        ...
        <h1 style="font-family:var(--font-serif);font-size:1.6rem;color:#fff;margin-bottom:20px;">Tout le réseau E-Benin</h1>
        ...
        <form action="{{ url('/search') }}" method="GET" id="search-form">
            <div style="display:flex;gap:8px;max-width:700px;">
                <div style="position:relative;flex:1;">
                    ...
                    <input type="text" name="q" value="{{ $query }}" placeholder="Rechercher un article, un sujet…"
            ...
            <div style="margin-top:14px;padding-bottom:18px;">
                ...
                <div id="adv-filters" class="{{ $activeFilters > 0 ? '' : 'hidden' }}"
                    ...
                    {{-- Blog --}}
                    <div>
                        <label style="display:block;font-size:.72rem;color:rgba(255,255,255,.6);font-weight:700;margin-bottom:4px;text-transform:uppercase;letter-spacing:.08em;">Blog</label>
                        <select name="blog" onchange="this.form.submit()"
                    ...
                    <div>
                        ...
                        <select name="rubrique" onchange="this.form.submit()"
                            ...
                            @foreach($rubriques as $rub)
                                <option value="{{ $rub->id }}" {{ ($rubriqueId ?? '') == $rub->id ? 'selected' : '' }}>
                                    {{ $rub->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:12px;padding-bottom:4px;">
                    @if($organizationId ?? false)
                        @php $orgName = $organizations->firstWhere('id', $organizationId)?->organization_name ?? ''; @endphp
                        <a href="{{ request()->fullUrlWithQuery(['blog' => null, 'page' => null]) }}"
                    ...
                    @endif
                    @if($rubriqueId ?? false)
                        @php $rubName = $rubriques->firstWhere('id', $rubriqueId)?->name ?? ''; @endphp
...
<main>
    <div class="container" style="padding-top:28px;padding-bottom:48px;">
            ...
            <div class="news-grid">
                ...
                <a href="{{ $postUrl($post) }}" class="card">
                    ...
                    <div class="card__footer">
                        ...
                        <div class="card__author">{{ $post->user->organization->organization_name ?? 'Rédaction' }}</div>
                    </div>
                </a>
                @endforeach
            </div>
        ...
        @else
            <div style="text-align:center;padding:56px 0;color:var(--muted);">
                <svg style="width:48px;height:48px;stroke:var(--border);fill:none;stroke-width:1.5;margin:0 auto 16px;display:block;" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <p style="font-size:.9rem;">Saisissez un terme pour rechercher sur tout le réseau.</p>
            </div>
        @endif
    </div>
</main>
